...
bool isFull(StackType* stack) { //스택이 다 찼는지 확인하는 함수
    if (stack->top >= 20) { //스택이 다 차면 true
        return true;
    }
    else { //다 차지 않았으면 false
        return false;
    }
}
...
element peek(StackType* stack) { //스택의 최상위 원소를 반환하는 함수
...
element pop(StackType* stack) { //스택에 있는 원소를 빼는 함수
...
int oper(char op) { //연산자의 우선 순위를 결정하는 함수
    switch (op)
    {
    case '(':
    case ')':
...
void changePostfix(char* infix, char* postfix) { //후위 표기법으로 변환
    ...
    while (*infix != NULL) //전위 표기법으로 표기된 수식이 null이 아닐 때까지 스택을 이용해 반복하며 후위 표기법으로
    {
        if (*infix == '(') { //수식에 여는 괄호가 있으면,
            ...
            while (!isEmpty(&stack) && (oper(*infix) <= oper(peek(&stack)))) { //수식의 연산자와 스택 안의 연산자의 우선순위를 확인한 다음,
